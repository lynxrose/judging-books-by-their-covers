#Source: A_K_Nain, Thank you for the wonderful code
...
def get_integrated_gradients(img_input, top_pred_idx, baseline=None, num_steps=50):
    ...
    if baseline is None:
        baseline = np.zeros((128,128,3)).astype(np.float32)
    else:
...
def random_baseline_integrated_gradients(
    ...
    for run in range(num_runs):
        baseline = np.random.random((128,128,3)) * 255
...
def get_integrated_gradients(img_input, top_pred_idx, baseline=None, num_steps=50):
    ...
    # having same size as the input image.
    if baseline is None:
        baseline = np.zeros((28,28,3)).astype(np.float32)
    else:
        baseline = baseline.astype(np.float32)
...
def gradForBook(title):
    #  1. Convert the image to numpy array
    img = get_img_array(title)
